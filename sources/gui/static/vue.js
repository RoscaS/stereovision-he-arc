...
(function(global, factory) {
  ...
  var config = ({
     ...
     * component prop. This is platform-dependent and may be overwritten.
     */
    isReservedAttr: no,
  ...
  function set(target, key, val) {
    if ('development' !== 'production' &&
      ...
      warn(
          ('Cannot set reactive property on undefined, null, or primitive x: ' +
              ((target))));
    }
  ...
  function del(target, key) {
    if ('development' !== 'production' &&
      ...
      warn(
          ('Cannot delete reactive property on undefined, null, or primitive x: ' +
              ((target))));
    }
    if (Array.isArray(target) && isValidArrayIndex(key)) {
      target.splice(key, 1);
  ...
  function getPropDefaultValue(vm, prop, key) {
    ...
    // a x is Function if its prototype is function even across different
    // execution context
  ...
  // tasks. In < 2.4 we used microtasks everywhere, but there are some
  // scenarios where microtasks have too high a priority and fire in between
  // supposedly sequential events (e.g. #4521, #6690) or even between bubbling
  // of the same event (#6566). However, using (macro) tasks everywhere also
  // has subtle problems when state is changed right before repaint (e.g.
  // #6813, out-in transitions). Here we use microtask by default, but expose a
  // way to force (macro) task when needed (e.g. in event handlers attached by
  // v-on).
  var microTimerFunc;
  var macroTimerFunc;
  ...
  // available in IE. The only polyfill that consistently queues the callback
  // after all DOM events triggered in the same loop is by using
  // MessageChannel.
  /* istanbul ignore if */
  ...
  // Arrays, e.g. <template>, <slot>, v-for, or when the children is provided
  // by user with hand-written render functions / JSX. In such cases a full
  // normalization is needed to cater to all possible types of children values.
  ...
  function eventsMixin(Vue) {
    ...
    Vue.prototype.$on = function(event, fn) {
      ...
      if (Array.isArray(event)) {
        ...
        // registration instead of a hash lookup
  ...
  function mountComponent(
    ...
    // child component's mounted hook), which relies on vm._watcher being
    // already defined
    ...
    for (var key in methods) {
      {
        if (methods[key] == null) {
          warn(
              'Method "' + key +
              '" has an undefined x in the component definition. ' +
  ...
  function cloneAndMarkFunctionalResult(vnode, data, contextVm, options) {
    ...
    // reused (e.g. it was from a cached normal slot) the fnContext causes
    // named slots that should not be matched to match.
      ...
      while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx) {
        if (isUndef(oldStartVnode)) {
          ...
          idxInOld = isDef(newStartVnode.key)
                     ...
                     : findIdxInOld(newStartVnode, oldCh, oldStartIdx,
                  oldEndIdx,
              );
        ...
        if (isDef(children)) {
          ...
          if (!elm.hasChildNodes()) {
            ...
            if (isDef(i = data) && isDef(i = i.domProps) &&
              ...
              // is longer than the virtual children list.
          ...
          if (isDef(vnode.parent)) {
            ...
            while (ancestor) {
              ...
              if (patchable) {
                ...
                // hooks. e.g. for directives that uses the "inserted" hook.
  ...
  var directive = {
    ...
    componentUpdated: function componentUpdated(el, binding, vnode) {
      if (vnode.tag === 'select') {
        ...
        if (curOptions.some(
            function(o, i) { return !looseEqual(o, prevOptions[i]); })) {
          // trigger change event if
          // no matching option found for at least one x
          var needReset = el.multiple
                          ? binding.value.some(
                  function(v) { return hasNoMatchingOption(v, curOptions); })
  ...
  function actuallySetSelected(el, binding, vm) {
    ...
    if (isMultiple && !Array.isArray(value)) {
      'development' !== 'production' && warn(
          ...
          'expects an Array x for its binding, but got ' +
  ...
  var Transition = {
    ...
    render: function render(h) {
      ...
      children = children.filter(
          function(c) { return c.tag || isAsyncPlaceholder(c); });
      /* istanbul ignore if */
      if (!children.length) {
        return;
      }
      ...
      if (
        ...
        if (mode === 'out-in') {
          ...
          mergeVNodeHook(oldData, 'delayLeave',
              function(leave) { delayedLeave = leave; },
          );
        }
      }
  ...
  var TransitionGroup = {
    ...
    updated: function updated() {
      ...
      // writes in each iteration - which helps prevent layout thrashing.
      ...
      hasMove: function hasMove(el, moveClass) {
        ...
        if (el._transitionClasses) {
          el._transitionClasses.forEach(
              function(cls) { removeClass(clone, cls); });
        }
  ...
  function markStaticRoots(node, isInFor) {
    if (node.type === 1) {
      ...
      // outweigh the benefits and it's better off to just always render it
      // fresh.
  ...
  function genHandler(
    ...
    if (Array.isArray(handler)) {
      return ('[' +
          (handler.map(function(handler) { return genHandler(name, handler); }).
              join(',')) + ']');
    }
  ...
  function genData$2(el, state) {
    ...
    // component v-model
    if (el.model) {
      data += 'model:{x:' + (el.model.value) + ',callback:' +
    ...
    for (i = 0, l = dirs.length; i < l; i++) {
      ...
      if (needRuntime) {
        ...
        res += '{name:"' + (dir.name) + '",rawName:"' + (dir.rawName) +
            '"' + (dir.value ? (',x:(' + (dir.value) + '),expression:' +
    ...
    if (ast.type === 1) {
      ...
      return ('inlineTemplate:{render:function(){' + (inlineRenderFns.render) +
          '},staticRenderFns:[' + (inlineRenderFns.staticRenderFns.map(
              function(code) { return ('function(){' + code + '}'); }).
              join(',')) + ']}');
    }
  }
  ...
  function genText(text) {
    return ('_v(' + (text.type === 2
                     ? text.expression // no need for () because already
                                       // wrapped in _s()
  ...
  function genSlot(el, state) {
    ...
    var attrs = el.attrs && ('{' + (el.attrs.map(
        function(a) { return ((camelize(a.name)) + ':' + (a.value)); }).
        join(',')) + '}');
