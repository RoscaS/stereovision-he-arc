...
import numpy as np
from external.stereovision.exceptions import ChessboardNotFoundError
...
class StereoCalibrator(object):
    ...
    def calibrate_cameras(self):
        ...
        (calib.cam_mats["left"], calib.dist_coefs["left"],
         ...
         calib.f_mat) = \
            cv2.stereoCalibrate(self.object_points,
        ...
        (calib.rect_trans["left"], calib.rect_trans["right"],
         ...
         calib.valid_boxes["right"]) = \
            cv2.stereoRectify(calib.cam_mats["left"],
                              calib.dist_coefs["left"],
                              calib.cam_mats["right"],
                              calib.dist_coefs["right"],
                              self.image_size,
        ...
        # RECTIFICATION REMAP (PER CAMERA)
        for side in ("left", "right"):
            (calib.undistortion_map[side],
             calib.rectification_map[side]) = \
                cv2.initUndistortRectifyMap(calib.cam_mats[side],
